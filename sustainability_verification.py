...
fold = 0
#repeatition
for dataset,feat_type in [('2024-GP',"DENSE"),('2024-apigraph',"APIGRAPH"),("malscanscb","malscanscb"),("malscan","malscan")]:
#for dataset,feat_type in [('2024-GP',"DENSE")]: #use this to verify on DENSE only
    print(dataset)
    data_id = "drebin"
    ...
    t_normalized[t_normalized>6] = 6 #Cap times
    ...
    if feat_type == "DENSE":
        ...
        y_test = y_test_
        y_train = y_train_
    elif feat_type == "HCC" or "scb" in feat_type or feat_type == "":
        X_train = X_train_
        X_test = X_test_
        y_test = y_test_
        y_train = y_train_
    ...
    epoch = 100
    method = ""#"crop0"#randomly drop part of features when training
    if dataset != "malscan":
        model_id = "nn"
        print(model_id)
        if True:#not os.path.exists(f"models/{model_id}_{year}_{fold}_{data_id}_{dataset}_{feat_type}_invariance.pkl"):
            nn = model_utils.train_model(model_id,data_id,X_train[:,:-1], y_train, x_val[:,:-1], y_val, epoch=epoch,method=method)#IID
        ...
        for _ in range(5):
            ...
            ff.write(f"Testing on {year} - G-MoE - {dataset}: {final_f1}, avg: {sum(final_f1)/len(final_f1)} \n") 
            ff.flush()
    ...
    model_id = "linearsvm"
    data_id = "drebin"
    if dataset == "2024-GP" and model_id == "linearsvm":
        print("Linear SVM")
        if not os.path.exists(f"models/{model_id}_{year}_{fold}_{data_id}_{dataset}_{feat_type}_invariance.pkl"):
            nn = model_utils.train_model(model_id,data_id,X_train_[:,:-1], y_train_, X_train_[:,:-1], y_train_, epoch=epoch)#IID
            model_utils.save_model(model_id, nn, "models/", f"{model_id}_{year}_{fold}_{data_id}_{dataset}_{feat_type}_invariance")
    ...
    if feat_type == "DENSE" and False:
        ...
        if True:#not os.path.exists(f"models/nn_time_{year}_{fold}_{data_id}.pkl"):
            ...
            nn = model_utils.load_model("nn", data_id, "models/", f"nn_time_{year}_{fold}_{data_id}",X_train.shape[1])
        final_f1 = evaluate(nn,X_test,y_test)
        ff.write(f"Testing on {year} - {model_id} - {dataset}: {final_f1}, avg: {sum(final_f1)/len(final_f1)} \n") 
        ff.flush()

        print("Original MOE")
        model_id = "Original MoE"

...
def dex2X(tmpDir, dexFile):
    ...
    baksmali = subprocess.Popen(
        ['java', '-Xmx256M', '-jar',  settings.BACKSMALI, "d", '-o', smaliLocation, dexFile])
    baksmali.wait()
...
def getActivities(sampleFile):
    ...
    manifest = subprocess.Popen(
        [settings.AAPT, 'd', 'xmltree', sampleFile, "--file",'AndroidManifest.xml'],
        stdout=subprocess.PIPE,
        stdin=subprocess.PIPE,
        stderr=subprocess.PIPE)
    manifest = manifest.communicate(0)[0].decode().split("\n")
...
def getFeatures(logFile, sampleFile):
    ...
    sampleInfos = subprocess.Popen([settings.AAPT, 'd', 'badging', sampleFile],
                                   ...
                                   stdin=subprocess.PIPE,
                                   stderr=subprocess.PIPE)
    sampleInfos = sampleInfos.communicate(0)[0].decode().split("\n")
    sampleInfos = list(map(lambda x:x.strip(),sampleInfos))
...
def getFilesInsideApk(sampleFile):
    ...
    xml = subprocess.Popen(["zipinfo", '-1', sampleFile],
                           stdout=subprocess.PIPE,
                           stdin=subprocess.PIPE,
                           stderr=subprocess.PIPE)
    xml = xml.communicate(0)[0].decode().split("\n")
...
def getNet(sampleFile):
    ...
    xml = subprocess.Popen(
        ...
        stdin=subprocess.PIPE,
        stderr=subprocess.PIPE)
    #print(f"{settings.AAPT} d xmltree {sampleFile} AndroidManifest.xml")
    xml = xml.communicate(0)[0].decode().split("\n")
    i = 0
    for line in xml:
...
def detect(smaliLocation):
    with open(settings.ADSLIBS, 'r') as f:
        #smaliPath = list(tuple(rec) for rec in csv.reader(f, delimiter=';'))
        smaliPath = list(csv.reader(f, delimiter=';'))  # 直接转换为列表即可
    fileList = list()
    detectedAds = list()
    for dirname, dirnames, filenames in os.walk(smaliLocation):
        for filename in filenames:
...
def createOutput(workingDir, appNet, appProviders, appPermissions, appFeatures,
    ...
    outpath = os.path.join('features/drebin/', run_id + '.json')

...
def data_iter(batch_size, features, labels=None, shuffle=True): # Deal with sparse iter
    ...
    if "toarray" in dir(features):
        for i in range(0, num_examples, batch_size):
            j = indices[i: min(i + batch_size, num_examples)]
            if labels is not None:
                yield (torch.FloatTensor(features[j].toarray()), torch.LongTensor(labels[j]))  
            else:
                yield torch.FloatTensor(features[j].toarray())
    else:
        for i in range(0, num_examples, batch_size):
            j = indices[i: min(i + batch_size, num_examples)]
            if labels is not None:
...
def train(X_train, y_train, x_test, y_test, batch_size, net, loss, optimizer, device, num_epochs, method='', *args):
    ...
    #whether to do density robust training, use method="density0/crop0/timedensity0/densityXX" to activate it.
    if "density" in method or 'crop' in method:
        ...
        if 'time' in method:
            #mask the time featue
            coredict['available_indicies'][-1] = 0
        transform_as_prob(coredict)#transform values and densities into sparse distribution
    for epoch in tqdm(range(num_epochs)):
        if hasattr(net, 'centers'):
            if epoch < 10:
                net.log_sigma.requires_grad = False
                net.centers.requires_grad = False
            else:
                net.log_sigma.requires_grad = True
                net.centers.requires_grad = True
        ...
        for X, y in train_iter:
            if 'density' in method:
                x_list = [X]
                y_list = [y]
                for ii in set(y_list):
                    X_filled = fill_density(X[y==ii].clone(), coredict, fn)
        ...
        if not isinstance(x_test, np.ndarray):
          ...
          % (epoch + 1, train_l_sum / batch_count, train_acc_sum / n, time.time() - start))
            ...
            continue
        #calculate val loss
        y_pred = predict(net, x_test, device=device, batch=1024, args=[])
...
def get_density_dict(x_train):
    ...
    for i in range(x_train.shape[1]):
        print(i,x_train.shape[1])
        coredict[i] = dict()
        if scipy.sparse.issparse(x_train):
            #print(x_train[:,i].todense().A)
            unique_value_counts = bincount(x_train[:,i].todense().A.reshape(-1))#convert into Tensor in case of precision inconsistency
        else:
            unique_value_counts = bincount(x_train[:,i])#convert into Tensor in case of precision inconsistency
        for v,c in unique_value_counts:
            coredict[i][v] = c
        coredict["min_density"][i] = min(coredict[i].values())/x_train.shape[0]
        #use this to indicate the value with max density, preventing from repetitive modification on the same feature due to the main value changed
        coredict['main_value'][i] = sorted(coredict[i].items(),key=lambda item:item[1])[-1][0]
    return coredict

def update_dict(coredict,x_train,i):
    coredict[i] = dict()
    if scipy.sparse.issparse(x_train):
        unique_value_counts = bincount(x_train[:,i].todense().A.reshape(-1))
    else:
        unique_value_counts = bincount(x_train[:,i])#convert into Tensor in case of precision inconsistency
    for v,c in unique_value_counts:
        coredict[i][v] = c
...
def convert_to_sparse(arr_list):
    data = []
    row_ind = []
    col_ind = []
    for row_idx, (values, indices, cols) in enumerate(arr_list):
        data.extend(values)
        row_ind.extend([row_idx] * len(values))
        col_ind.extend(indices)
    rows = len(arr_list)
    #cols = max(col_ind) + 1 if col_ind else 0
    return csr_matrix((data, (row_ind, col_ind)), shape=(rows, cols))

class LongTailScaler:
    ...
    def transform(self, X):
        X = np.asarray(X, dtype=np.float32)
        norm = (X - self.min_val) / self.range_val
        compressed = np.power(norm, self.alpha)
    ...
    def inverse_transform(self, X_compressed):
        X_compressed = np.asarray(X_compressed, dtype=np.float32)
        norm = np.power(X_compressed, 1.0 / self.alpha)
        original = norm * self.range_val + self.min_val
        return original
